...
using T_OnPacket = VOID(_fastcall*)(PVOID pThis, PVOID edx, int nType, PVOID iPacket);
...
bool OnPacket(int nType, PVOID iPacket) {
...
VOID _fastcall hook_CLogin__OnPacket(PVOID pThis, PVOID edx, int nType, PVOID iPacket) {
    if (OnPacket(nType, iPacket)) return;
    orig_CLogin__OnPacket(pThis, edx, nType, iPacket);
}

VOID _fastcall hook_CField__OnPacket(PVOID pThis, PVOID edx, int nType, PVOID iPacket) {
    if (OnPacket(nType, iPacket)) return;
    orig_CField__OnPacket(pThis, edx, nType, iPacket);
}

VOID _fastcall hook_CCashShop__OnPacket(PVOID pThis, PVOID edx, int nType, PVOID iPacket) {
    if (OnPacket(nType, iPacket)) return;
    orig_CCashShop__OnPacket(pThis, edx, nType, iPacket);
}

VOID _fastcall hook_CITC__OnPacket(PVOID pThis, PVOID edx, int nType, PVOID iPacket) {
    if (OnPacket(nType, iPacket)) return;
    orig_CITC__OnPacket(pThis, edx, nType, iPacket);
}

VOID _fastcall hook_CWvsContext__OnPacket(PVOID pThis, PVOID edx, int nType, PVOID iPacket) {
    if (OnPacket(nType, iPacket)) return;
